 ...
 * slaxHandler.js will call saveReferer(details) when it is about to redirect to
...
// g_referrers[tabId][frameId] = referrer of slax frame.
var g_referrers = {};
...
// to matching slax resource requests (only if the Referer is non-empty). The
// handler is removed as soon as the slax viewer frame is unloaded.
chrome.runtime.onConnect.addListener(function onReceivePort(port) {
  ...
  // If the slax is viewed for the first time, then the referer will be set here.
  var referer = g_referrers[tabId] && g_referrers[tabId][frameId] || '';
  port.onMessage.addListener(function(data) {
    // If the viewer was opened directly (without opening a slax URL first), then
